...
let gameMode = 'pvcpu'; // 'pvcpu', 'pvp', ou 'campaign' // ALTERADO
...
document.getElementById('campaignButton').addEventListener('click', () => {
    ...
    p1CharSelectTitle.textContent = "Campanha: Escolha seu Lutador";
});
...
function confirmCharacter(playerNumber) {
    ...
    } else if (playerNumber === 2) { // Apenas para modos não-campanha
...
function generateCampaignRoster() {
    ...
    const opponentPool = availableCharacters.filter(char => char.id !== selectedP1Char.id);

    // Embaralha o array de oponentes
    for (let i = opponentPool.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [opponentPool[i], opponentPool[j]] = [opponentPool[j], opponentPool[i]];
    }

    // Pega os primeiros 6 para a campanha
    campaignOpponents = opponentPool.slice(0, CAMPAIGN_NUM_FIGHTS);
}

function startCampaign() {
    generateCampaignRoster();
    campaignStage = 0;
    startCampaignFight();
}

function startCampaignFight() {
    if (campaignStage >= campaignOpponents.length) {
        // Vitória na Campanha!
        gameOverMessage.textContent = 'CAMPANHA CONCLUÍDA! VOCÊ É O CAMPEÃO!';
        gameOverModal.style.display = 'flex';
        return;
    }

    // Define o próximo oponente
    selectedP2Char = campaignOpponents[campaignStage];
    // Define a dificuldade da fase atual
    selectedDifficulty = CAMPAIGN_DIFFICULTIES[campaignStage];
    // Seleciona uma arena aleatória
    const randomArenaIndex = Math.floor(Math.random() * availableArenas.length);
    selectedArenaBg = availableArenas[randomArenaIndex].bgPath;
...
function startGame() {
    ...
    if (gameMode === 'campaign') {
        if (!selectedP1Char || !selectedP2Char) return;
        opponentName = selectedP2Char.name;
        opponentLabel = `CPU (LUTA ${campaignStage + 1})`;
    } else {
    ...
    if(p2ControlsDisplayDiv) {
        ...
        if(p2H3) p2H3.textContent = `${opponentLabel}: ${opponentName.toUpperCase()}`;
            ...
            setTimeout(() => {
                if (winnerIndex === 0) { // Jogador venceu a luta
                    ...
                    if (campaignStage >= campaignOpponents.length) {
                        ...
                        messageArea.textContent = `VITÓRIA! Preparando próximo oponente...`;
                        setTimeout(startCampaignFight, ROUND_END_DELAY);
                    }
                } else { // Jogador perdeu a luta
                    gameOverMessage.textContent = `FIM DA CAMPANHA! Você foi derrotado por ${roundWinnerPlayer.name.toUpperCase()}.`;
                    gameOverModal.style.display = 'flex';
                }
            }, 1000);
        } else { // Lógica para outros modos
...
function gameLoop(timestamp) {
    ...
    if (roundOverState) {
        ...
        if (roundOverTimer <= 0 && !matchOver) { // Não avança se a partida terminou
            currentRound++;
            if (currentRound > MAX_ROUNDS || player1RoundWins >= Math.ceil(MAX_ROUNDS/2) || player2RoundWins >= Math.ceil(MAX_ROUNDS/2) ) {
                // A partida acabou, a lógica em handleRoundEnd já foi chamada.
            } else {
                startNewRound();
            }
        }
